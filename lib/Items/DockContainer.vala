...
namespace Plank {
  ...
  public abstract class DockContainer : DockElement {
    ...
    bool enable_anchor_logic (DockItem? item, DefaultApplicationDockItemProvider? provider) {
      ...
      if ((!(item is DockletItem) && provider.Prefs.AnchorDocklets) ||
          (!(item is FileDockItem) && provider.Prefs.AnchorFiles)) {
        return true;
      }
    ...
    bool check_anchor_item (DockElement element, DefaultApplicationDockItemProvider provider) {
      if ((element is DockletItem && provider.Prefs.AnchorDocklets) ||
          (element is FileDockItem && provider.Prefs.AnchorFiles)) {
        return true;
      }
    ...
    void add_without_signaling (DockElement element) {
      ...
      if (enable_anchor_logic (item, provider)) {
        ...
        if (item is TransientDockItem) {
          ...
          for (int i = internal_elements.size - 1; i >= 0; i--) {
            if (check_anchor_item (internal_elements[i], provider)) {
              insert_pos = i;
            } else {
              break;
            }
