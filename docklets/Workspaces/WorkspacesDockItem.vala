...
namespace Docky {
  public class WorkspacesDockItem : DockletItem {
    private ulong workspaces_prefs_handler_id = 0;
    ...
    private Gdk.RGBA active_color;
    private Gdk.RGBA inactive_color;
    private int rows;
    private int cols;
    ...
    construct
    {
      ...
      active_color = { 1.0f, 1.0f, 1.0f, 1.0f }; // Pure white
      inactive_color = { 0.1f, 0.1f, 0.1f, 0.9f }; // Almost black

      setup_workspace_info ();

      workspaces_prefs_handler_id = workspaces_prefs.notify["InvertColor"].connect (() => {
        queue_redraw ();
      });

      connect_screen_signals ();

      workspaces_prefs.notify["LivePreviews"].connect (() => {
    ...
    ~WorkspacesDockItem () {
      if (workspaces_prefs_handler_id > 0) {
        workspaces_prefs.disconnect (workspaces_prefs_handler_id);
        workspaces_prefs_handler_id = 0;
      }
    ...
    protected override void draw_icon (Surface surface) {
      ...
      int cell_size = int.min (max_cell_width, max_cell_height);

      int grid_width = cell_size * cols;
      int grid_height = cell_size * rows;
      ...
      for (int i = 0; i < workspace_count; i++) {
        ...
        draw_workspace (cr, x, y, cell_size, cell_size, i);
      }
    }
        ...
        if (workspace != null) {
          foreach (unowned Wnck.Window window in screen.get_windows ()) {
            if (window.is_on_workspace (workspace) &&
              ...
              if (workspaces_prefs.InvertColor) {
                if (workspace_num == current_workspace) {
                  window_color = { 0.7, 0.7, 0.7, 0.7 };
                } else {
                  window_color = { 0.0, 0.0, 0.0, 0.7 };
                }
              } else {
                if (workspace_num == current_workspace) {
                  window_color = { 0.3, 0.3, 0.3, 0.7 };
                } else {
                  window_color = { 1.0, 1.0, 1.0, 0.7 };
                }
              }
